...
namespace ithare {
	namespace mtprimitives {
		...
		//helpers for MWSRQueue_FC
		//*** mt_mask_*() functions need to represent a coherent view, but nobody really cares about exact bit numbers outside of them ***//
		bool mt_mask_getbit(uint64_t mask, int pos) {
			assert(pos >= 0);
			assert(pos < 64);
			return (mask & (uint64_t(1) << pos)) != 0;
		}

		uint64_t mt_mask_setbit(uint64_t mask, int pos) {
			assert(pos >= 0);
			assert(pos < 64);
			return mask | (uint64_t(1) << pos);
		}

		bool mt_mask_resetbit(uint64_t mask, int pos) {
			assert(pos >= 0);
			assert(pos < 64);
			return mask & ~(uint64_t(1) << pos);
		}

		uint64_t mt_mask_shiftoutbit0(uint64_t mask) {
			return mask >> 1;
		}
		...
		class ExitReactorHandle {
#ifndef ITHARE_MTPRIMITIVES_CAS_DUMMY
			static_assert(sizeof(ExitReactorData<QueueSize>) == CAS_SIZE, "size of ReactorData MUST match CAS_SIZE");
#endif
		...
		public:
			...
			bool writeCompleted(uint64_t id) {
				while (true) {
					...
					assert(!mt_mask_getbit(mask, int(id - firstR)));
					uint64_t newMask = mt_mask_setbit(mask, int(id - firstR));
			...
			uint64_t readCompleted(uint64_t id) {
				...
				while (true) {
					...
					uint64_t newMask = mt_mask_shiftoutbit0(mask);
					new_data.setCompletedWritesMask(newMask);
		...
		***/
		...
		public:
			MWSRQueue()
				: entrance(EntranceReactorData(
					ExitReactorData<QueueSize>::EntranceFirstToWrite, 
					ExitReactorData<QueueSize>::EntranceLastToWrite).getCasData()) {
			}
			void push(QueueItem&& item) {
				...
				ExitReactorHandle<QueueSize> ex(exit);
